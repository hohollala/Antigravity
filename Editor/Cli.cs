...
namespace Antigravity.Editor
{
	internal static class Cli
	{
		...
		internal static string GetInstallationDetails(IAntigravityBaseInstallation installation)
		{
		...
		internal static void GenerateSolutionWith(AntigravityExternalEditor vse, string installationPath)
		{
			if (vse != null && vse.TryGetAntigravityBaseInstallationForPath(installationPath, lookupDiscoveredInstallations: true, out var vsi))
			{
		...
		{
			if (CodeEditor.CurrentEditor is AntigravityExternalEditor vse)
			{
			...
			else
			{
				...
				try
				{
					var installations = Discovery
						...
						.Cast<AntigravityBaseInstallation>()
					...
					if (installation != null)
					{
						...
						try
						{
							...
							GenerateSolutionWith(CodeEditor.CurrentEditor as AntigravityExternalEditor, installation.Path);
						}
						finally
						{
